 ...
 * @version 1.0
 * 
 *          This class is a simple implementation of a plotter. It accepts two arrays of float
 *          values which represent the x- and y-coordinates of points which should be plotted in a
 *          carthesian coordinate system. x- and y-axis are scaled logarithmically. No interpolation
 *          between the dots is done!
 */
public class Plotter extends JPanel {
	...
	/**
	 * Constructor of this class. Assigns the passed x- and y-values of the points to plot to the
	 ...
	 * @throws InstantiationException The lengths of the x- and y-coorinates arrays have to be equal.
	 *            Elsewise an exception is thrown.
	 */
	public Plotter(float[] xData, float[] yData)
	...
	protected void paintComponent(Graphics g) {
		...
		/* draw x-axis */
		graphics.draw(new Line2D.Double(PADDING, PADDING, PADDING,
			height - PADDING));
		graphics.draw(new Line2D.Double(PADDING + 0.25f
			* (width - 2 * PADDING), height - PADDING - 5, PADDING
			+ 0.25f * (width - 2 * PADDING), height - PADDING + 5));
		graphics.draw(new Line2D.Double(0.5f * width, height - PADDING
			- 5, 0.5f * width, height - PADDING + 5));
		graphics.draw(new Line2D.Double(PADDING + 0.75f
			* (width - 2 * PADDING), height - PADDING - 5, PADDING
			+ 0.75f * (width - 2 * PADDING), height - PADDING + 5));
		graphics.draw(new Line2D.Double(width - PADDING, height
			- PADDING - 5, width - PADDING, height - PADDING + 5));
		
		/* draw x-axis caption */
		graphics.drawString(new Float(minX).toString(), PADDING + 2,
			height - PADDING + 12);
		graphics.drawString(
		...
		graphics.drawString(
			new Float(Math.pow(10.0d, 0.75f * Math.log10(maxX) + 0.25f
				...
				* (width - 2 * PADDING) + 2, height - PADDING + 12);
		graphics.drawString(new Float(maxX).toString(), width - PADDING
			+ 2, height - PADDING + 12);
		
		/* draw y-axis */
		graphics.draw(new Line2D.Double(PADDING, height - PADDING,
		...
		for (float element : xData) {
			x = scaleX(element, width);
			y = scaleY((float) (1.0d / Math.sqrt(element)), height);
			graphics.fill(new Ellipse2D.Float(x - 1.0f, y - 1.0f, 2.0f,
				2.0f));
		}
	...
	/**
	 * The x-values are logarithmically scaled before they are drawn. This is done by this method.
	 * The padding of the coordinate system is respected.
	 * 
	 * @param x Value which should be scaled logarithmically.
	 * @param width Width of the plottable area.
	 * @return Logarithmically scaled value of x.
	 */
	private float scaleX(float x, float width) {
		float xScale = (width - 2 * PADDING)
			/ (float) (Math.log10(maxX) - Math.log10(minX));
		float xOffset = (float) -Math.log10(minX);
		float result = PADDING + xScale
			* (xOffset + (float) Math.log10(x));
		
		return result;
	}
	
	/**
	 * The y-values are logarithmically scaled before they are drawn. This is done by this method.
	 * The padding of the coordinate system is respected.
	 * 
	 * @param y Value which should be scaled logarithmically.
